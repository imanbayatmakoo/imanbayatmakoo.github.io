...
function renderMetricsLadder(metrics) {
  ...
  const rows = metrics
    .map((m) => {
      ...
      const move = m.value
        ? esc(m.value)
        : `${esc(m.before)} → ${esc(m.after)}`;
      return `
        <div class="ladder-row">
          <span class="ladder-label">${esc(m.label)}</span>
          <span class="ladder-move">${move}</span>
...
function renderRoleBody(r) {
  if (r.sections && r.sections.length) {
    return r.sections
      .map(
        (sec) => `
        <h4 class="role-sub">${esc(sec.heading)}</h4>
        <ul>${(sec.bullets || []).map((li) => `<li>${esc(li)}</li>`).join("")}</ul>`
      )
      .join("");
  }
  return `<ul>${(r.bullets || []).map((li) => `<li>${esc(li)}</li>`).join("")}</ul>`;
}
...
function renderResume(cv) {
  ...
  const skillCols = (cv.skills || [])
    .map(
      (g) =>
        `<div><h3>${esc(g.name)}</h3><ul>${(g.keywords || [])
          .map((k) => `<li>${esc(k)}</li>`)
          .join("")}</ul></div>`
    )
    .join("");
  ...
  document.getElementById("resume").innerHTML = `
    ...
    <div class="cred-grid">
      ...
      ${skillCols}
      ${tools ? `<div><h3>Tools</h3><ul>${tools}</ul></div>` : ""}
...
function renderContact(cv) {
  ...
  document.getElementById("contact").innerHTML = `
    ...
    <div class="contact-links">
      ...
      ${link(c.phone ? "tel:" + c.phone.replace(/[^+\d]/g, "") : "", c.phone)}
